...
@dp.message(F.text.regexp(r"^ва-банк\s*([кч]|на\s*красное|на\s*чёрное|красное|чёрное|к|ч|\d+)$", flags=re.IGNORECASE))
...
async def spin_roulette(message: types.Message):
    ...
    if user_id not in current_bets:
        await message.answer(f"⚠️ KNK-{user_name}, сначала сделайте ставку! Например: `2000 к` же `10000 7`")
        return
        
    bet = current_bets[user_id]
    last_bets[user_id] = bet.copy()
    
    # Секундомер 7 секунд назад (как на вашем скриншоте)
    ...
    # Удаляем сообщение с секундомером вместо него отправляем GIF анимацию рулетки
    try:
        await msg.delete()
    except:
        pass

    # Отправляем GIF рулетки (вы можете заменить эту публичную ссылку на file_id вашей GIF)
    gif_url = "https://media.giphy.com/media/xT5LMRQ628iuRMVMwo/giphy.gif"
    gif_msg = None
    try:
        gif_msg = await message.answer_animation(animation=gif_url)
    except:
        gif_msg = await message.answer("🎲 [Гифка рулетки крутится...]")

    # GIF крутится 3 секунды
    ...
    # Проверка выигрыша (поддерживает ставку на точное число или цвет)
    is_win = False
    ...
    if is_win:
        ...
        response += f"❌ KNK-{user_name} проиграл {bet['amount']} 🌒"
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if data == "bet_red_1k":
        ...
        if user_id not in current_bets:
            ...
            return
        # Для кнопки вызовем логику кручения через псевдо-сообщение
