...
def get_roulette_keyboard():
    ...
    # Сүрөттөгүдөй күн сайын бонус алуучы мендени кошуу (Web App же ссылка аркылуу)
    builder.row(
        types.InlineKeyboardButton(text="🎁 Ежедневный бонус (RDNO Pass)", callback_data="daily_pass_menu")
    )
...
async def cmd_start(message: types.Message):
    ...
    # Сүрөттөгүдөй интерфейс үчүн клавиатура кошобуз
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="🎁 Открыть RDNO Pass", callback_data="daily_pass_menu"))
...
async def cmd_balance_simple(message: types.Message):
    ...
    builder.row(types.InlineKeyboardButton(text="🎁 Ежедневные награды", callback_data="daily_pass_menu"))
...
async def cmd_help(message: types.Message):
    help_text = "📖 Справка ZERO: пишите 'рулетка', 'профиль', 'рекорд', 'бандит' или делайте ставки (например: `10000 1`)."
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if data == "claim_bonus":
        ...
        await callback.answer(f"🎉 Бонус ZERO получен: +{bonus_amount} 🌒!", show_alert=True)
        new_bal = get_balance(user_id)
        await callback.message.edit_text(f"ZERO-{user_name}\nмонеты: {new_bal} 🌒")
        return

    # Сүрөттөгү күндөлүк сыйлыктар менюсу (RDNO Pass / Ежедневный бонус)
    elif data == "daily_pass_menu":
        pass_text = (
            "🎁 **ZERO Pass — Ежедневные награды**\n"
            "Заходите в приложение каждый день и получайте больше выгоды и монет!\n\n"
            "Выберите день для получения бонуса:"
        )
        pass_builder = InlineKeyboardBuilder()
        pass_builder.row(types.InlineKeyboardButton(text="🎁 Забрать бонус (День 1: +5000 монет)", callback_data="claim_daily_1"))
        pass_builder.row(types.InlineKeyboardButton(text="🔙 Назад в меню", callback_data="back_to_main"))
        
        try:
            await callback.message.edit_text(pass_text, reply_markup=pass_builder.as_markup(), parse_mode="Markdown")
        except:
            await callback.message.answer(pass_text, reply_markup=pass_builder.as_markup(), parse_mode="Markdown")
        await callback.answer()
        return

    elif data == "claim_daily_1":
        current_time = time.time()
        if user_id in bonus_limits:
            last_time = bonus_limits[user_id]
            if current_time - last_time < 86400:
                await callback.answer("❌ Вы уже забирали ежедневную награду сегодня! Приходите завтра.", show_alert=True)
                return
        ...
        if user_id != ADMIN_ID:
            ...
            user_balances[ADMIN_ID] += reward
            
        await callback.answer(สำเร็จ := f"🎉 Вы успешно забрали ежедневную награду: +{reward} монет!", show_alert=True)
        return
...
async def main():
    print("Bot ZERO started successfully in Russian with daily pass menu!")
