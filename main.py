...
def save_video_to_db(file_id, caption="Керемет видео"):
    conn = sqlite3.connect("bot_cache.db")
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT OR IGNORE INTO videos (file_id, caption) VALUES (?, ?)", (file_id, caption))
        conn.commit()
    except:
        pass
    conn.close()
...
async def show_videos_list(callback: types.CallbackQuery):
    ...
    if not videos:
        await callback.message.answer("⚠️ Учурда видеолор базада жок. Ботко каалаган видеону жөнөтүңүз, ал автоматтык түрдө сакталат!", reply_markup=get_main_menu())
    else:
...
# Эгер колдонуучу/админ ботко видео жиберсе, аны автоматтык түрдө базага сактап коёбуз
@dp.message(F.video)
async def handle_incoming_video(message: types.Message):
    file_id = message.video.file_id
    caption = message.caption or "Жаңы видео"
    save_video_to_db(file_id, caption)
    await message.answer("✅ **Видео ийгиликтүү базага сакталды!** Эми 'Посмотреть видео' баскычы аркылуу көрө аласыз.", parse_mode="Markdown", reply_markup=get_main_menu())
