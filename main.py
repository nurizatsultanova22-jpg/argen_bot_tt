...
async def handle_business_message(message):
    ...
    # 1. Эгер Арген (сиз) өзүңүз жооп берсеңиз
    if is_outgoing:
        replied_chats.add(chat_id)
        user_message_counts[user_id] = 0  # Санагычты нолго түшүрөбүз
        return

    # Эгер Арген жооп берген болсо жана азырынча жаңы сүйлөшүү баштала элек болсо
    ...
    # 3. Үчүнчү билдирүүгө жооп (башкача текст)
    elif count == 3:
        text_3 = (
        ...
        try:
            ...
            pass
        # Андан кийинкилерин кайталабашы үчүн санагычты токтотобуз же башынан баштаса болот
