...
async def start_game_handler(message: types.Message):
    ...
    # Скриншоттогудай столбик түрүндөгү тарыхты түзүү
    history_str = "\n".join(user["roulette_history"])

    # Утуш же уттуруу жыйынтыгын жазуу
    if is_win:
        winnings = bet_amt * win_mult
        user["balance"] += winnings
        user["total_won_coins"] += winnings
        result_text = f"🎉 <b>{username}</b> выиграл(а) <b>{winnings}</b> монет на {rolled_emoji}!"
        ...
        result_text = f"❌ <b>{username}</b> проиграл(а) <b>{bet_amt}</b> монет на {rolled_emoji}."

        if bet_amt > global_records["max_loss"]["amount"]:
            global_records["max_loss"] = {"name": username, "amount": bet_amt}

    # Толук жыйынтыкты жана тарыхты чыгаруу
    final_output = f"{history_str}\n\n{result_text}"
    await message.answer(final_output, parse_mode="HTML")
...
async def show_game_logs(message: types.Message):
    ...
    history_display = "📜 <b>История последних игр:</b>\n\n" + "\n".join([f"{item}" for item in history])
