...
# --- ПРОФИЛЬ / БАЛАНС ---
@dp.message(F.text.casefold().in_(["б", "профиль", "мой профиль"]))
...
# --- СТАВКИ В РУЛЕТКЕ (КАТАРЫ МЕНЕН КААЛООГО ЖОРОГОН СТАВКАЛАР ҮЧҮН) ---
@dp.message(F.text.casefold().regexp(r'^\d+\s+([0-12]|[кчз]|на\s+[а-яА-ЯёЁ]+)$'))
async def save_bet_handler(message: types.Message):
    ...
    # Сүрөттөгүдөй ырастама кайтаруу же тынч кабыл алуу (бул жерде колдонуучу катары менен көп ставка коё алгыдай кылынды)

# --- ЗАПУСК РУЛЕТКИ («ГО» / «КРУТИТЬ») С ТАЙМЕРОМ И GIF АНИМАЦИЕЙ ---
...
async def start_game_handler(message: types.Message):
    ...
    # КАЗИНО GIF АНИМАЦИЯСЫ (иштеши үчүн туураланган ссылка)
    try:
    ...
    # СҮРӨТТӨГҮДӨЙ ЖЫЙЫНТЫК ФОРМАТЫ (Аты + Ставкалар тизмеси)
    result_lines = [
        f"<b>ZERO 🎰🇰🇬</b>\n",
    ...
    for bet in bets:
        ...
        # Сүрөттөгүдөй чыгаруу: Игрок аты + сумма + на [сан/түс]
