...
async def cmd_start(message: types.Message):
    ...
    # Сиз сурангандай орус тилинде, киргенде дароо чыгуучу маалымат
    text = (
        ...
        "• `ID + 10000` — перевести монеты по ID (лимит 1 раз в 24 часа)\n\n"
...
@dp.message(F.text.regexp(r"^(\d+)\s*\+\s*10000$", flags=re.IGNORECASE))
async def transfer_money_by_id(message: types.Message):
    text = message.text.strip()
    match = re.match(r"^(\d+)\s*\+\s*10000$", text, re.IGNORECASE)
    if not match:
        return
    ...
    await message.answer(
        f"✅ Успешный перевод!\n"
        f"✈️ KNK-{sender_name} перевел 10000 монет пользователю с ID: `{target_id}` 🌕\n"
...
# Рулетканы айландыруу (GIF чыгып, 5 секундтан кийин өчүп, натыйжа чыгат)
...
async def spin_roulette(message: types.Message):
    ...
    # GIF билдирүүнү толугу менен өчүрүү (удалить кылуу)
    try:
...
# Интерактивдүү кнопкалар аркылуу ойноо (GIF анимациясы менен)
@dp.callback_query()
