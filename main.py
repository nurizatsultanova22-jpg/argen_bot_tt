...
# YouTube жана SoundCloud'ду катар колдонуучу универсалдуу жөндөө
YDL_OPTS_SEARCH = {
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "✨ Салам! Мен сиздин музыкалык ботуңузмун! 🎵🤖\n\n"
        "🎧 Каалаган ырдын атын түздөн-түз жазып издеңиз! ⚡️\n"
        "🔥 Же төмөнкү кнопканы басып, хит тректерди тандаңыз 👇\n\n"
        f"👑 Админ: {ADMIN_USER} 💻"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query == "🔥 Популярдуу тректер (ТОП-10) 🌟":
        ...
        await update.message.reply_text("🔥 Тренддеги ТОП-10 тректер: 🎧✨", reply_markup=InlineKeyboardMarkup(keyboard))
        return

    search_query = query.strip()
    status_msg = await update.message.reply_text("🔍 Музыка изделүүдө... ⏳")
    
    track_info = None
    try:
        # Биринчи YouTube'дан издеп көчүрөбүз
    ...
    # Эгер YouTube таппай калса, SoundCloud'дан кошумча издейбиз
    if not track_info:
        try:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pop_"):
        ...
        status_msg = await query.message.reply_text("⚡️ Тандалган трек изделүүдө... ⏳")
    ...
    status_msg = await query.message.reply_text(f"⏳ **{track['title']}** жогорку сапатта жүктөлүүдө... 🚀")
