...
async def search_music_options(message: types.Message):
    ...
    try:
        ...
        for idx, entry in enumerate(entries[:3], 1):
            ...
            # Которуу максатында callback_data аркылуу ссылканы кыскача сактайбыз же URL беребиз
            builder.row(InlineKeyboardButton(text=f"🎵 Вариант {idx}: {title[:35]}...", callback_data=f"dl_{entry.get('id')}"))
...
# --- ТАНДАЛГАН ВАРИАНТТЫ ЖҮКТӨП БЕРҮҮ (320kbps + ФОН) ---
@dp.callback_query(F.data.startswith("dl_"))
async def download_selected_track(callback: types.CallbackQuery):
    video_id = callback.data.split("_")[1]
    video_url = f"https://www.youtube.com/watch?v={video_id}"
    
    await callback.message.edit_text("⏳ **Скачиваем трек в высоком качестве (320kbps) с фоном...** 🎶", parse_mode="Markdown")
