...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        "🎧 **Spotify стилиндеги музыкалык ботко кош келиңиз!** ✨\n\n"
        "Ыр издөө үчүн жөн гана атын жазыңыз же **'трек [аты]'** / **'найти [аты]'** форматтарын колдонуңуз. "
        "Бот сизге 3 вариант сунуштайт! 🎶"
    )
...
async def btn_warm_words(message: types.Message):
    compliments = [
        "💖 **Бүгүнкү күнүңүз ийгиликтүү улансын!** ✨",
        "🌸 **Сиз эң мыкты нерселерге татыктуусуз!**",
        "✨ **Алга карай гана умтулуңуз!** 🔥"
    ]
...
async def btn_entertainment(message: types.Message):
    await message.answer("🎲 **Мотивация:** Ийгилик — бул кичинекей кадамдардын жыйындысы! 🔥", parse_mode="Markdown", reply_markup=get_main_reply_menu())
...
# --- МУЗЫКА ИЗДӨӨ (ТРЕК / НАЙТИ СӨЗДӨРҮ МЕНЕН ЖЕ ТҮЗ) ---
@dp.message()
async def search_music_options(message: types.Message):
    ...
    # "трек" же "найти" сөздөрүн алып салып, таза атын чыгарабыз
    query = text
    ...
    waiting_msg = await message.answer("🎧 **Тректер изделүүдө (3 вариант)...** 🎶", parse_mode="Markdown")
    ...
    try:
        def search_tracks():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # SoundCloud аркылуу үзгүлтүксүз 3 вариант издейбиз
        ...
        for idx, entry in enumerate(entries[:3], 1):
            ...
            url = entry.get("url")
            if url:
                builder.row(InlineKeyboardButton(text=f"🎵 Вариант {idx}: {title[:35]}...", callback_data=f"dl_{url[:50]}"))

        await bot.delete_message(message.chat.id, waiting_msg.message_id)
        
        if builder.buttons:
            await message.answer(
                f"🔍 **'{query}'** боюнча варианттар табылды. Керектүүсүн тандаңыз:",
                parse_mode="Markdown",
...
async def download_selected_track(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("⏳ **Жогорку сапаттагы трек жүктөлүүдө (320kbps)...** 🎶", parse_mode="Markdown")
