...
# --- Экрандын астындагы төрт бурчтуу кнопкалар (Reply Keyboard) ---
...
# «б» же «баланс» деп жазганда скриншоттогудай монеталар чыгат
@dp.message(F.text.lower().in_({"б", "баланс"}))
async def cmd_balance_simple(message: types.Message):
    user_id = message.from_user.id
    bal = get_balance(user_id)
    await message.answer(f"{bal} 🌕")

# Профиль толук маалымат үчүн
...
async def play_bandit(message: types.Message):
    ...
    # Слот анимациясы же эмодзилер катары
    slot_emojis = ["🍇", "🍋", "🍊", "🍒", "7️⃣"]
