...
async def cmd_bandit(message: types.Message):
    ...
    try:
        await msg.edit_text(f"🎰 **Однорукий бандит**\n👤 {user_name}•🪶\n\n{res1} {box_white} {box_white} {box_white} {box_white}")
    except:
        pass
        
    await asyncio.sleep(0.8)
    try:
        await msg.edit_text(f"🎰 **Однорукий бандит**\n👤 {user_name}•🪶\n\n{res1} {res2} {box_white} {box_white} {box_white}")
    except:
        pass
        
    await asyncio.sleep(0.8)
    try:
        await msg.edit_text(f"🎰 **Однорукий бандит**\n👤 {user_name}•🪶\n\n{res1} {res2} {res3} {box_white} {box_white}")
    except:
        pass
        
    await asyncio.sleep(0.8)
    try:
        await msg.edit_text(f"🎰 **Однорукий бандит**\n👤 {user_name}•🪶\n\n{res1} {res2} {res3} {res4} {box_white}")
    except:
        pass
    ...
    if len(set(win_symbols)) == 1:
        win_msg = "🎉 ДЖЕКПОТ! Вы выиграли 50000 🌒!"
        if user_id != ADMIN_ID:
            user_balances[user_id] = get_balance(user_id) + 50000
        else:
            user_balances[ADMIN_ID] += 50000
    elif len(set(win_symbols)) <= 3:
        win_msg = "✨ Отлично! Вы выиграли 10000 🌒!"
        if user_id != ADMIN_ID:
            user_balances[user_id] = get_balance(user_id) + 10000
        else:
            user_balances[ADMIN_ID] += 10000
    else:
...
async def parse_bet(message: types.Message):
    ...
    await message.answer(f"✅ **Ставка принята!**\n👤 {user_name}•🪶\n🎯 Сумма: **{amount} 🌒** ({color_text})", parse_mode="Markdown")
...
async def spin_roulette(message: types.Message):
    ...
    # 1. 3 секунддук санақ
    countdown_msg = await message.answer(f"👤 {user_name}•🪶 крутит (через 3 сек.)")
    for sec in range(2, 0, -1):
        await asyncio.sleep(1)
        try:
            await countdown_msg.edit_text(f"👤 {user_name}•🪶 крутит (через {sec} сек.)")
        except:
            pass
    ...
    # 2. Санақ бүткөндөн кийин GIF сүрөттү (анимацияны) чыгыруу
    gif_url = "https://media.giphy.com/media/3oKIPnAiaMCws8nOsE/giphy.gif" # Анимация шилтемеси
    gif_msg = None
    try:
    ...
    # Айлануу процессине 2-3 секунд убакыт берүү
    ...
    for u_id, bets_list in active_bets_copy.items():
        ...
        for bet in bets_list:
            ...
            if is_win:
                ...
                response += f"🎉 ZERO-{u_name} выиграл **+{win_amount}** на {winning_result}\n"
            else:
                if is_green and b_color != "зелёный":
                    ...
                    response += f"🔄 ZERO-{u_name}: Возврат **{refund}** (Зеро)\n"
                else:
                    stats['lost'] += b_amount
                    response += f"❌ ZERO-{u_name} проиграл **{b_amount}**\n"
                    
        bal_current = get_balance(u_id)
        response += f"💳 Баланс: {bal_current}\n"
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if data == "daily_pass_menu":
        ...
        await callback.message.answer(f"✅ **Ставка удвоена!**\n👤 {user_name}•🪶\n🎯 Сумма: **{double_amount} 🌒** ({new_lb['text']})", parse_mode="Markdown")
...
async def main():
    print("Bot started successfully without ads!")
