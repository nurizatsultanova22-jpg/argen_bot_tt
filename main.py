...
# YouTube'дан издөө параметрлери
YDL_OPTS_SEARCH = {
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "✨ Салам! Мен сиздин YouTube музыкалык ботуңузмун! 🎵🤖\n\n"
        "🎧 Каалаган ырдын атын түздөн-түз жазып, YouTube'дан заматта издеп алыңыз! ⚡️\n"
        "🔥 Же төмөнкү кнопканы басып, акыркы хит тректерди тандаңыз 👇\n\n"
        f"👑 Админ: {ADMIN_USER} 💻"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query == "🔥 Популярдуу тректер (ТОП-10) 🌟":
        keyboard = [
            ...
            [InlineKeyboardButton("🎵 Macan - Asphalt 8", callback_data="pop_6")],
        ...
        await update.message.reply_text("🔥 YouTube базасынан тренддеги ТОП-10 тректер: 🎧✨", reply_markup=InlineKeyboardMarkup(keyboard))
        return

    search_query = query.strip()
    status_msg = await update.message.reply_text("🔍 YouTube'дан тез изделүүдө... ⏳")
    ...
    try:
        ...
        await status_msg.edit_text("❌ Тилекке каршы, YouTube'дан эч нерсе табылган жок. Башка атын жазып көрүңүзчү! 🔍✨")
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("pop_"):
        ...
        status_msg = await query.message.reply_text("⚡️ YouTube'дан тез изделүүдө... ⏳")
    ...
    status_msg = await query.message.reply_text(f"⏳ **{track['title']}** YouTube'дан жогорку сапатта жүктөлүүдө... 🚀")
