...
async def download_and_send_audio(chat_id, query, context, is_auto=False):
    # SoundCloud же Deezer аркылуу издөө блокталбайт
    ydl_opts = {
        'format': 'bestaudio/best',
        'noplaylist': True,
        'default_search': 'scsearch1',  # SoundCloud аркылуу издөө
        'postprocessors': [{
    ...
    try:
        ...
        print(f"Ошибка: {e}")
        if not is_auto:
            await context.bot.send_message(chat_id=chat_id, text="Табылбады. Башка ырды жазып көрүңүз.")
