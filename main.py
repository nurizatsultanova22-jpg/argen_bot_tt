...
def search_tracks(query):
    # Тректи табуу үчүн бир нече булактан издейбиз (биринчи YouTube, таппаса SoundCloud)
    results = []
    ydl_opts = {
    ...
    # Эгер YouTube'дан чыкпай калса, SoundCloud'дон кошуп издейбиз
    if len(results) < 3:
        try:
            ydl_opts_sc = {
            ...
            with yt_dlp.YoutubeDL(ydl_opts_sc) as ydl:
                ...
                for entry in res.get('entries', []):
                    title = entry.get('title')
                    url = entry.get('url')
                    if title and url and not any(r['title'] == title for r in results):
                        results.append({'title': title, 'url': url})
        except:
            pass
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        ...
        "🚀 Ырдын атын жазсаңыз, сүрөтү жана толук файлы менен таап берем!\n\n"
