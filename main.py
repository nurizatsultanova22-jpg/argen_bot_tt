...
# Яндекс Музыка же башка ачык API аркылуу издөө (Жеке yt-dlp'нин yandexmusic search кыймылдаткычы)
YDL_OPTS_SEARCH = {
    'default_search': 'ymsearch1', # Яндекс Музыкадан 1 так вариантты табуу
    'quiet': True,
    'extract_flat': True,
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text("🔎 Яндекс Музыкадан изделүүдө...")
    
    try:
        with yt_dlp.YoutubeDL(YDL_OPTS_SEARCH) as ydl:
            res = ydl.extract_info(f"ymsearch1:{query}", download=False)
