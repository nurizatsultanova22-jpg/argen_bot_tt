...
current_bets = {}  
last_user_bets = {} 
user_history = {} # Оюндардын историясы үчүн
...
async def cmd_start(message: types.Message):
    ...
    # Реферал системасы (дос чакыруу аркылуу 100,000 монета берүү)
    ...
    text = (
        ...
        "Используйте меню ниже для игры в рулетку и слоты:"
    )
...
async def cmd_balance_simple(message: types.Message):
    ...
    # Администраторго закреп / статус көрсөтүү
...
async def cmd_bandit(message: types.Message):
    ...
    bet_amount = 50000 # Стандартная ставка слота
    ...
    if len(set(win_symbols)) == 1:
        ...
        add_history(user_id, f"Ставка (слот): -{bet_amount}")
        add_history(user_id, f"Выигрыш (слот): +{win_amt}")
        ...
        add_history(user_id, f"Ставка (слот): -{bet_amount}")
        add_history(user_id, f"Проигрыш (слот)")
...
async def main():
    print("Bot started successfully with professional style and auto-records!")
