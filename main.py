...
# 7 секунд санап, андан кийин видео/гиф чыгып, 3 секунддан кийин өчүп, жыйынтык чыгуучу бөлүгү
...
async def spin_roulette(message: types.Message):
    ...
    # Видеодогудай монеталар түшкөн GIF же видео ссылкасы
    gif_url = "https://media.giphy.com/media/3oKIPnAiaMCws8nOsE/giphy.gif"
    
    gif_msg = None
    try:
        gif_msg = await message.answer_animation(animation=gif_url, caption="💰 Монеты падают...")
    except Exception:
        gif_msg = await message.answer("💰 Монеты падают...")

    # Гиф туура 3 секунд турат
    await asyncio.sleep(3.0)

    # 3 секундтан кийин гифти өчүрүү
    try:
        if gif_msg:
