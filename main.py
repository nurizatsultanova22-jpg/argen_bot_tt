...
# Словарь для отслеживания активности игроков за день (ID -> количество игр)
daily_activity = {}
...
# Список запрещенных слов (мат и грубые выражения)
BAD_WORDS = {
    "зайбал", "заебал", "заебался", "котогум", "котокум", "сука", "блять", 
    "блядь", "нахуй", "нахауй", "нахуй", "энендин", "еблан", "далбайоп", 
...
# --- АНТИМАТ И АНТИСПАМ ССЫЛОК ДЛЯ ГРУПП ---
...
async def group_moderation(message: types.Message):
    ...
    for word in words:
        ...
        if cleaned_word in BAD_WORDS:
            is_bad = True
            break
        # Проверка по вхождению подстрок для жесткого мата
    ...
    if is_bad:
        ...
        # Отправляем предупреждение с реплаем (отметкой) и стикером
        sent_msg = await message.answer(warning_text)
        try:
            # Отправка стикера с предупреждением (айди стикера можно заменить по желанию)
            await message.answer_sticker("CAACAgIAAxkBAAEZ73tm_X...", reply_to_message_id=message.message_id)
        except Exception:
            pass

# Проверка и обновление топ-активности игрока за день
...
# Функция для определения самого активного игрока дня
def get_most_active_user():
    if not daily_activity:
        return None
    top_user_id = max(daily_activity, key=daily_activity.get)
    return top_user_id

# --- /START ---
@dp.message(F.text.lower() == "/start")
async def cmd_start(message: types.Message):
    await message.answer("👋 Бот активен и готов к игре! Напишите «б», «Бандит» или «Рулетка».")
...
async def slot_bandit_game(message: types.Message):
    ...
    # Проверяем, является ли игрок самым активным сегодня для значка 💪
...
async def cmd_roulette(message: types.Message):
    ...
    text = (
        ...
        "7🔴 8⚫ 9🔴 10⚫ 11🔴 12⚫\n\n"
        "Примеры ставок:\n"
        "• 5000 2 (на конкретное число)\n"
        "• 30000 к (на красное)\n"
        "• 30000 ч (на черное)\n"
        "• ва банк к (или ва-банк ч / ва банк 5)"
    )
    await message.answer(text, reply_markup=get_roulette_keyboard())

# СТАВКА В РУЛЕТКЕ (включая ВА БАНК / ВА-БАНК)
@dp.message(F.text.regexp(r'(?i)^(ва[- ]банк|\d+)\s+(\d+|[кчзКЧЗ]|на\s+(красное|черное|зеленое))'))
async def save_bet_handler(message: types.Message):
    ...
    if "банк" in parts[0] or "банк" in text:
        ...
        if bet_amt <= 0:
            await message.answer("❌ У вас нет монет для ставки ва-банк!")
            return
    ...
    display_amount = "ВА БАНК" if ("банк" in text) else str(bet_amt)
    await message.answer(f"<b>{username}</b> 🕊 {display_amount} {target}", parse_mode="HTML")
...
async def start_game_handler(message: types.Message):
    ...
    if user_id not in users_data or not users_data[user_id].get("pending_bets"):
        await message.answer("⚠️ Сначала сделайте ставки (например: 5000 2 или ва банк к)!")
        return
    ...
    if user_id not in ADMIN_IDS and user_data_dict["balance"] < total_bet_amt:
        await message.answer("❌ Недостаточно монет для всех ставок!")
    ...
    # Добавляем эмодзи 💪 самому активному игроку дня
    activity_badge = " 💪" if get_most_active_user() == user_id else ""
