...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        "🟢 **Добро пожаловать в SoundCloud музыкальный бот!** 🎧\n\n"
...
# --- ТОЛЬКО SOUNDCLOUD ЧЕРЕЗ API ---
async def search_soundcloud_track(query):
    # SoundCloud публичный API аркылуу тректин шилтемесин таап беребиз
    api_url = f"https://soundcloud.com/oembed?format=json&url=https://soundcloud.com/search?q={query}"
    async with aiohttp.ClientSession() as session:
        try:
            # Түз эле SoundCloud'дун сунушталган шилтемесин же yt-dlp аркылуу soundcloud URL издегенге өбөлгө түзөбүз
            search_target = f"https://soundcloud.com/search/sounds?q={query}"
...
async def search_music_options(message: types.Message):
    ...
    # Тек SoundCloud менен гана иштөөчү yt_dlp жөндөөлөрү (YouTube кошулган эмес)
    ydl_opts = {
    ...
    if not success or not os.path.exists(filename):
        await message.answer("❌ В SoundCloud ничего не найдено. Попробуйте написать название точнее.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        return
