...
# --- «Б» ЖАЗГАНДА БОЛГОНУ МОНЕТАНЫ ГАНА КӨРСӨТҮҮ ---
@dp.message(F.text.casefold().in_(["б", "в"]))
async def show_user_coins_only(message: types.Message):
    user_id = message.from_user.id
    ...
    await message.answer(f"💰 {display_balance}", parse_mode="HTML")

# --- «ВА БАНК» - БАРДЫК МОНЕТАНЫ СТАВКАҒА КОЮУ ---
@dp.message(F.text.casefold() == "ва банк")
async def all_in_bet_handler(message: types.Message):
    user_id = message.from_user.id
    ...
    if user_id in ADMIN_IDS:
        await message.answer(f"<b>ZERO 🎰🇰🇬</b>\n\n⚠️ Админтер ва банк ойной албайт!", parse_mode="HTML")
        return

    balance = user["balance"]
    if balance <= 0:
        await message.answer(f"<b>ZERO 🎰🇰🇬</b>\n\n❌ <b>{username}</b>, балансыңызда монета калган жок!", parse_mode="HTML")
        return

    # Рандомдуу түрде сан же түс тандап бардык монетаны ставка кылат (мисалы, кызылға же караға)
    chosen_target = random.choice(["к", "ч", str(random.randint(0, 12))])
    
    bet_type = None
    bet_desc = ""

    if chosen_target.isdigit():
        bet_type = f"num_{chosen_target}"
        bet_desc = f"на {chosen_target}"
    elif chosen_target == "к":
        bet_type = "red"
        bet_desc = "на красное"
    else:
        bet_type = "black"
    ...
    await message.reply(f"🔥 ВА БАНК! Ставка принята: <b>{balance}</b> на <b>{chosen_target}</b>", parse_mode="HTML")
...
async def save_bet_handler(message: types.Message):
    ...
    await message.reply(f"✅ Ставка принята: <b>{bet_amt}</b> на <b>{target}</b>", parse_mode="HTML")

# --- ЗАПУСК РУЛЕТКИ («ГО» / «КРУТИТЬ») С ТАЙМЕРОМ И GIF ---
...
async def start_game_handler(message: types.Message):
    ...
    try:
        gif_msg = await message.answer_animation("https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExM3Blb3Z5b2I5aWx3MnhmODZ5bXF6a2F5dXB2NXJ0M3h6OGFqZmZ0ZSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/l0HlRnAWXxn0MhOBK/giphy.gif")
...
async def show_bandit_history(message: types.Message):
    if not bandit_daily_history:
        await message.answer("<b>ZERO 🎰🇰🇬</b>\n\n📜 История игр за день пуста!", parse_mode="HTML")
        return
