...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        ...
        "Чтобы найти трек, обязательно напишите в начале **«Трек [название]»** или **«Найти [название]»**! 🎶"
    )
...
async def search_music_options(message: types.Message):
    ...
    waiting_msg = await message.answer("🟢 **Ищем трек в высоком качестве...** 🎧", parse_mode="Markdown")

    filename = f"audio_{random.randint(10000, 99999)}.mp3"
    
    # yt-dlp жөндөөлөрү (бир нече жерден издөө үчүн)
    ydl_opts = {
    ...
    try:
        def download_file():
            ...
            # Алгач SoundCloud, эгер таппаса YouTube аркылуу издейт
            search_methods = [f"scsearch1:{query}", f"ytsearch1:{query}"]
            
            for method in search_methods:
                try:
                    current_opts = ydl_opts.copy()
                    current_opts["default_search"] = method.split(":")[0]
                    with yt_dlp.YoutubeDL(current_opts) as ydl:
                        info = ydl.extract_info(method, download=True)
                        if info:
    ...
    if not success or not os.path.exists(filename):
        await message.answer("❌ Трек не найден. Попробуйте написать название точнее.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        return
