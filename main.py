...
def search_tracks(query):
    results = []
    # 1. Сүрөт жана аталыш үчүн iTunes (Deezer сыяктуу тез база) колдонобуз
    try:
    ...
    # 2. Эгер iTunes таппай калса, SoundCloud'дон издейбиз
    if not results:
        try:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("select_"):
        ...
        status_msg = await query.message.reply_text(f"⚡ SoundCloud'дан файл алынып жатат: *{title}* ⏳", parse_mode="Markdown")

        try:
            # Файлды SoundCloud аркылуу толук түшүрүп алабыз
            ydl_opts = {
                'format': 'bestaudio/best',
                'default_search': f"scsearch:{title}",
                'quiet': True,
                'noplaylist': True
            }
            audio_url = None
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(f"scsearch:{title}", download=False)
                if 'entries' in info:
                    info = info['entries'][0]
            ...
            try:
                await status_msg.edit_text("❌ Аудиофайлын табууга болбоду. Башка тректи көрүңүз.")
            except:
