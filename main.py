...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        ...
        "Бот сизге 3 вариантты тез таап берет! 🎶"
    )
...
# --- БӨГӨТКӨ КАБЫЛБАГАН ИЗДӨӨ ЖАНА ЖҮКТӨӨ ---
@dp.message()
    ...
    try:
        def search_tracks():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # Jiosaavn аркылуу эч кандай блоксуз 3 вариант алабыз
                results = ydl.extract_info(f"jiosaavnsearch3:{query}", download=False)
        ...
        for idx, entry in enumerate(entries[:3], 1):
            ...
            url = entry.get("url")
            if url:
                builder.row(InlineKeyboardButton(text=f"🟢 Вариант {idx}: {title[:35]}...", callback_data=f"dl_{url[:60]}"))

        await bot.delete_message(message.chat.id, waiting_msg.message_id)
        
        if builder.buttons:
            await message.answer(
                f"🟢 **'{query}'** боюнча варианттар табылды. Тандаңыз:",
                parse_mode="Markdown",
...
async def download_selected_track(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text("⏳ **Трек жогорку сапатта жүктөлүүдө...** 🎧", parse_mode="Markdown")
