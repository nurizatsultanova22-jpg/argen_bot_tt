...
async def group_moderation(message: types.Message, bot: Bot):
    ...
    # Ссылкаларды жана рекламаларды текшерүү (Админден башкаларга тыюу салынат)
...
async def cmd_roulette(message: types.Message):
    ...
    text = (
        ...
        "<code>1000 на красное</code> | <code>5000 на 12</code> | <code>10000 к</code>"
    )
    await message.answer(text, reply_markup=get_roulette_inline_keyboard(), parse_mode="HTML")

# --- СТАВКИ В РУЛЕТКЕ (ТЕКСТОМ) ---
@dp.message(F.text.casefold().regexp(r'^(ва\s*банк|\d+\s*к?)\s+([0-12]|[кчз]|на\s+(красное|черное|зеленое))'))
async def save_bet_handler(message: types.Message):
    ...
    raw_amt_str = parts[0]
    if "банк" in raw_amt_str or "ва" in raw_amt_str:
    ...
    display_amount = f"{bet_amt}"
    await message.answer(f"<b>ZERO 🎰🇰🇬</b>\n\n✅ <b>{username}</b> 🕊 Ставка принята: {display_amount} на {target}", parse_mode="HTML")
...
async def start_game_handler(message: types.Message):
    ...
    # СТАВКА ПРИНЯТА + ТАЙМЕР 3, 2, 1 СЕКУНД
