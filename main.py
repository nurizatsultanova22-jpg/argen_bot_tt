...
async def cmd_start(message: types.Message):
    await message.answer("👋 Саламатсызбы! Бот иштеп жатат. «Рулетка» же «Бандит» деп жазып оюнду баштасаңыз болот.")

# --- ID КОМАНДАСЫ ---
@dp.message(F.text.lower() == "/id")
async def show_user_id(message: types.Message):
    user_id = message.from_user.id
    username = message.from_user.first_name or message.from_user.username | "Пользователь"
...
async def slot_bandit_game(message: types.Message):
    ...
    bet_cost = 1000  # Бир жолку ойноо баасы (монета)
    ...
    # 5 ак түстөгү төрт бурчтуктар менен башталышы
    msg = await message.answer(f"🎰 <b>{username}</b> крутит слот...\n⬜⬜⬜⬜⬜", parse_mode="HTML")
    await asyncio.sleep(1.2)

    # Символдор тизмеси
    symbols = ["♥️", "♦️", "♠️", "🀄", "🎴"]
    
    # 5 торчо үчүн кокус символдорду тандоо
    result_symbols = [random.choice(symbols) for _ in range(5)]
    result_display = "".join(result_symbols)

    try:
        await msg.edit_text(f"🎰 <b>{username}</b> крутит слот...\n{result_display}", parse_mode="HTML")
    except Exception:
        pass

    await asyncio.sleep(1.0)

    # Утуштарды эсептөө (кайсы символ канча жолу чыкканын санап чыгабыз)
    ...
    # 5 🎴 же 4 🎴 -> 5000 (шартыңызда чыкса 5000 денилген)
    elif counts["🎴"] >= 4:
        win_amount = 5000
        win_desc = "4 карта (🎴) үчүн 5 000 монета!"

    final_text = f"🎰 <b>СЛОТЫ — БАНДИТ</b>\n{result_display}\n\n"

    if win_amount > 0:
        user["balance"] += win_amount
        user["total_won_coins"] += win_amount
        final_text += f"🎉 Поздравляем! Вы выиграли <b>{win_amount}</b> монет ({win_desc})"
        ...
        final_text += f"😢 К сожалению, вы проиграли 1000 монет. Попробуйте еще раз!"
