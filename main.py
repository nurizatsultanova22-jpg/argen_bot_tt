...
# --- ИГРА «БАНДИТ» (5 БЕЛЫХ КВАДРАТОВ И ПОЭТАПНОЕ ОТКРЫТИЕ) ---
@dp.message(F.text.lower() == "бандит")
async def slot_bandit_game(message: types.Message):
    user_id = message.from_user.id
    username = user.from_user.first_name or message.from_user.username or "Игрок"
    ...
    # 1. Сначала показываем 5 белых пустых квадратиков
    msg = await message.answer(f"💎 <b>{username}</b>\n\n⬜ ⬜ ⬜ ⬜ ⬜", parse_mode="HTML")
    await asyncio.sleep(0.8)

    # Символы для слотов (пика, маджонг, трефа, бубна, червы)
    possible_symbols = ["♠️", "🀄", "♣️", "♦️", "♥️"]
    
    # Генерируем 5 случайных символов
    res_symbols = [random.choice(possible_symbols) for _ in range(5)]

    # Постепенное открытие по одному символу слева направо (как на скриншоте)
    current_display = ["⬜", "⬜", "⬜", "⬜", "⬜"]
    for i in range(5):
...
async def save_bet_handler(message: types.Message):
    ...
    if user_id not in ADMIN_IDS and user["balance"] < bet_amt:
        await message.answer("❌ У недостаточно монет на балансе!")
        return
