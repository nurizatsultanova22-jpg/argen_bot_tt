...
async def search_music_options(message: types.Message):
    ...
    try:
        def download_file():
            ...
            # SoundCloud издөө үчүн альтернативдүү форматты колдонобуз
            search_query = f"scsearch:{query}"
            
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # Эгер scsearch иштебесе, тикелей SoundCloud издөө шилтемесин колдонобуз
                try:
                    info = ydl.extract_info(search_query, download=True)
                except:
                    info = ydl.extract_info(f"https://soundcloud.com/search/sounds?q={query.replace(' ', '%20')}", download=True)
    ...
    if not success or not os.path.exists(filename):
        await message.answer("❌ Трек не найден в SoundCloud. Попробуйте другое название.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        return
