...
# "Бандит" командасы: 5 ак түстөгү төрт бурчтук катары менен ачылат
...
async def cmd_bandit(message: types.Message):
    ...
    # Баштапкы 5 ак түстөгү төрт бурчтуктар
    box_white = "⬜"
    ...
    await asyncio.sleep(0.8)
    # 5-төрт бурчтук толук ачылып жыйынтык чыгарылат
...
async def spin_roulette(message: types.Message):
    ...
    gif_msg = None
    try:
        gif_msg = await message.answer_animation(animation=gif_url, caption="🎲 Вращаем рулетку...")
    except Exception:
        gif_msg = await message.answer("🎲 Вращаем рулетку...")
