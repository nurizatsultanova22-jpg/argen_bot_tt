...
# --- 1. СТАВКА ЖАЗГАНДА (Текст же кыскартылган формат) ---
...
async def save_bet_handler(message: types.Message):
    ...
    if "к" in side or "красное" in side:
        bet_type = "red"
        bet_name = "на красное"
    elif "ч" in side or "черное" in side:
        bet_type = "black"
        bet_name = "на черное"
    elif "з" in side or "зеленое" in side:
        bet_type = 0
        bet_name = "на зеленое"
    else:
        return
...
async def start_game_handler(message: types.Message):
    ...
    if user_id not in users_data or not users_data[user_id].get("pending_bet"):
        await message.answer("⚠️ Сначала сделайте ставку (например: 1000 к же 1000 на красное)!")
        return
    ...
    # Скриншоттогудай тагыраак жыйынтык форматы
    result_lines = [
...
# --- «ЛОГ» ЖЕ «ИСТОРИЯ» ДЕП ЖАЗГАНДА СТРАНИЦАДАГЫЧА СТОЛБИК ЧЫККАН ЖЕР ---
