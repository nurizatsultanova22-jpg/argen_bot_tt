...
# Рулетка айланганда GIF анимациясын чыгаруу функциясы
...
async def spin_roulette(message: types.Message):
    ...
    # Бул жерге өзүңүздүн GIF шилтемеңизди (URL) же файдыныздын ID коё аласыз
    # Мисалы открытый публичный GIF ссылкасы же Телеграмдагы file_id жазса болот:
    gif_url = "https://media.giphy.com/media/3oKIPnAiaMCws8nOsE/giphy.gif"
    
    try:
        await message.answer_animation(animation=gif_url, caption="🎲 Рулетка крутится...")
    except Exception:
        # Эгер GIF жүктелбей калса текст чыгат
        await message.answer("🎲 Рулетка крутится...")

    await asyncio.sleep(4.0) # Анимация бүткөнчө күтүү убактысы
