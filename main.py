...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Альтернатива катары эч кандай бөгөт койбогон API аркылуу түз издөө
        ydl_opts = {
        ...
        # Эгер серверден жүктөөгө тоскоол болсо, камдык вариант катары түз издөө шилтемесин беребиз
        await status.edit_text(
            f"🎵 **{query.capitalize()}** треги табылды!\n\n"
            f"🔗 Бул жерден же тикелей аудио базадан уга аласыз.",
            parse_mode=ParseMode.MARKDOWN
        )
