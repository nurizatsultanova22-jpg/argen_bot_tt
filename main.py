...
async def parse_vabank(message: types.Message):
    ...
    await message.answer(f"KNK-{user_name} {amount} {color_text}")
...
async def spin_roulette(message: types.Message):
    ...
    # Отправляем GIF рулетки с анимацией
    anim_msg = await message.answer_animation(
        animation="CgACAgQAAxkBAAICa2W... (ссылка или дефолтная анимация кубика/слота)",
        caption="🎲 Рулетка крутится..."
    )
    # Если анимация не загрузилась по ссылке, используем замену на текст-анимацию
    if not anim_msg:
        anim_msg = await message.answer("🎰 Рулетка крутится...")
    ...
    try:
        await anim_msg.edit_caption(caption=response)
    except:
        await message.answer(response)
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if data == "bet_red_1k":
        ...
        await callback.message.answer(f"KNK-{user_name} 1000 на красное")
        ...
        if user_id != ADMIN_ID:
            user_balances[user_id] -= 1000
        await callback.message.answer(f"KNK-{user_name} 1000 на чёрное")
        ...
        if user_id != ADMIN_ID:
            user_balances[user_id] -= 1000
        await callback.message.answer(f"KNK-{user_name} 1000 на зелёный")
