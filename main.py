...
# Рулетка айланып жатканда көрсөтүлө турган GIF же анимациянынลิงги (ачык булактагы же өзүңүздүн GIF шилтемеңиз)
...
async def spin_roulette(message: types.Message):
    ...
    # Рулетка айланып баштаганда GIF анимациясын жиберебиз
    ...
    # Гифти өчүрүп же ордуна жыйынтык чыгарабыз
    try:
    ...
    for u_id, bets_list in active_bets_copy.items():
        ...
        for bet in bets_list:
            ...
            if is_win:
                ...
                response += f"👤 {u_name}•🪶: 🏆 Выиграл **+{win_amount} 🌒** ({winning_result})\n"
            else:
                if is_green and b_color != "зелёный":
                    ...
                    response += f"👤 {u_name}•🪶: 🔄 Возврат **{refund} 🌒** (Зеро)\n"
                else:
                    stats['lost'] += b_amount
...
async def main():
    print("Bot started successfully with Roulette GIF!")
