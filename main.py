...
# Популярдуу ырлардын алдын ала даярдалган тизмеси (Хиттер)
POPULAR_TRACKS = [
    "Bakr - Самая близкая",
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = (
        ...
        "🚀 Отправь название трека, часть текста или воспользуйся кнопкой ниже, чтобы увидеть топ треков.\n\n"
        "🔥 Команды:\n"
        "• Напиши название или текст песни\n"
...
async def top_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Тизмени кэшке салып, баскыч аркылуу жүктөөгө даярдайбыз
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status_msg = await update.message.reply_text("⚡ Ищу трек по названию или тексту... 🔍")
    ...
    response_text = "🎵 Самые лучшие результаты:\n\n"
    keyboard = []
    row = []
    ...
    await status_msg.edit_text(response_text + f"\n👑 Администратор: {ADMIN_USER}", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("select_"):
        ...
        status_msg = await query.message.reply_text(f"📥 Загружаю трек: {title} ⏳")
