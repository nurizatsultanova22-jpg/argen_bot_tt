...
# YouTube'дун катасын бербес үчүн эң акыркы жана күчтүү жөндөө
YDL_OPTIONS = {
    'format': 'best',
    'quiet': True,
    'no_warnings': True,
    'nocheckcertificate': True,
    'extractor_args': {'youtube': {'player_client': ['android', 'web']}}, # Sign in катасын чечет
    'http_headers': {
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Привет! Напиши название песни или видео, я быстро найду его! 🎬", reply_markup=reply_markup)
...
async def search_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    status = await update.message.reply_text("🔍 Ищу на YouTube...")
...
async def play_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await status.edit_text("❌ Ошибка при скачивании видео.")
