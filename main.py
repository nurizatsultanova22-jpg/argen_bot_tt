...
async def cmd_start(message: types.Message):
    ...
    await message.answer("Салам! Жаңы бот иштеп жатат.", reply_markup=builder.as_markup(resize_keyboard=True))
...
async def callback_handler(callback: types.CallbackQuery):
    ...
    if data == "bet_red_1k":
        ...
        await callback.message.answer("Ставка принята: 1000 на красное")
    elif data == "bet_black_1k":
        current_bets[user_id] = {"amount": 1000, "color": "чёрное"}
        await callback.message.answer("Ставка принята: 1000 на чёрное")
