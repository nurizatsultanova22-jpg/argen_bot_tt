...
search_cache = {}
ai_active_users = set()  # AI режими күйгөн колдонуучуларды сактоо үчүн
...
async def ask_ai(prompt_text):
    try:
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "Сен жардамчы ботсуң. Жоопторду абдан так, түшүнүктүү, абзацтарга бөлүп жана ар бир сүйлөмгө же ойго тиешелүү сонун эмодзилерди кошуп кыргыз тилинде жаз."},
...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in ai_active_users:
        ai_active_users.remove(user_id) # Башынан баштаганда AI режимин өчүрүп турабыз
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Кнопка басылганда AI режимин күйгүзүү
    ...
    # 3. Эгер колдонуучунун AI режими КҮЙҮК болсо — бардык жазган сөзүнө AI жооп берет
    if user_id in ai_active_users:
    ...
    # 4. Кадимки музыка издөө (трек / ыр)
    ...
    # Эгер эч бирине туура келбесе
    await update.message.reply_text("💡 Музыка издөө үчүн башына **'трек [аты]'** же **'ыр [аты]'** деп жазыңыз.\n\n🤖 Же **'🤖 AI менен сүйлөшүү'** кнопкасын басып маектешиңиз.", parse_mode="Markdown")
