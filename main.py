...
# --- ИНТЕРАКТИВДҮҮ МЕНЮ (Катар кылып экиден топтолду) ---
def get_main_menu():
    builder = InlineKeyboardBuilder()
    # 1-катар: Календарь жана Видео
    builder.row(
        InlineKeyboardButton(text="📅 Календарь", callback_data="show_calendar"),
        InlineKeyboardButton(text="🎬 Видео (24ч)", callback_data="show_videos_list")
    )
    # 2-катар: Жылуу сөз жана Зериккенде
    builder.row(
        InlineKeyboardButton(text="💬 Жылуу сөз", callback_data="warm_words"),
        InlineKeyboardButton(text="🎲 Зериккенде", callback_data="fun_entertainment")
    )
    # 3-катар: Группа жана Админ
    builder.row(
        InlineKeyboardButton(text="💎 Наша группа", url="https://t.me/taanyshuu_chatyy"),
        InlineKeyboardButton(text="⚡️ АДМИН", url="https://t.me/Argen_70")
    )
