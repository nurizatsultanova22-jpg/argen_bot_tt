...
async def cmd_start(message: types.Message):
    await message.answer("👋 Саламатсызбы! Бот иштеп жатат. «Профиль», «Рулетка» же «Бандит» деп жазып көрсөңүз болот.")
...
async def show_user_profile(message: types.Message):
    ...
    profile_text = (
        ...
        f"💎 Премиум: {'Бар (VIP)' if user_id in ADMIN_IDS else 'Жок'}\n"
        f"💰 Монеталар балансы: {display_balance}\n"
        f"📊 Утуп алган монеталар: {u_data['total_won_coins']}\n"
        f"📉 Утулуп калган монеталар: {u_data['total_lost_coins']}\n\n"
        f"━━━━━━━━━━━━━━━\n"
        f"🛡 Статус: <b>{ 'Администратор 👑' if user_id in ADMIN_IDS else 'Активдүү оюнчу ⭐' }</b>"
    )
...
# --- «БАНДИТ» ОЮНУ (СЕКИН ЖАНА НАТИЖАСЫ ТАК КӨРСӨТҮЛӨТ) ---
...
async def slot_bandit_game(message: types.Message):
    ...
    if user_id not in ADMIN_IDS and user["balance"] < bet_cost:
        await message.answer("❌ У вас недостаточно монет для игры в «Бандит» (нужно 1000 монет)!")
        return

    if user_id not in ADMIN_IDS:
        user["balance"] -= bet_cost

    msg = await message.answer(f"🎰 <b>{username}</b> | Бандит крутится...\n⏳ До начала: 3...", parse_mode="HTML")
    await asyncio.sleep(1)
    try:
        await msg.edit_text(f"🎰 <b>{username}</b> | Бандит крутится...\n⏳ До начала: 2...", parse_mode="HTML")
    except Exception:
        pass
    await asyncio.sleep(1)
    try:
    ...
    symbols = ["♥️", "♦️", "♠️", "🀄", "🎴"]
    for _ in range(2):
        temp_display = "".join([random.choice(symbols) for _ in range(5)])
        try:
            await msg.edit_text(f"🎰 <b>{username}</b> крутит...\n{temp_display}", parse_mode="HTML")
        except Exception:
            pass
        await asyncio.sleep(0.6)

    result_symbols = [random.choice(symbols) for _ in range(5)]
    result_display = "".join(result_symbols)

    counts = Counter(result_symbols)
    win_amount = 0
    win_desc = ""

    if counts["♥️"] >= 3:
        win_amount = 3000
        win_desc = "3 жүрөк (♥️) үчүн 3 000 монета!"
    elif counts["♦️"] >= 4:
        win_amount = 6000
        win_desc = "4 бубей (♦️) үчүн 6 000 монета!"
    elif counts["♠️"] >= 4:
        win_amount = 6000
        win_desc = "4 пика (♠️) үчүн 6 000 монета!"
    elif counts["🀄"] >= 4:
        win_amount = 10000
        win_desc = "4 маджонг (🀄) үчүн 10 000 монета!"
    elif counts["🎴"] >= 4:
        win_amount = 5000
        win_desc = "4 карта (🎴) үчүн 5 000 монета!"

    final_lines = [
        f"🎰 <b>СЛОТЫ — БАНДИТ</b>",
        f"Комбинация: <b>{result_display}</b>",
        f"👤 <b>{username}</b> ставка: 1000 монет"
    ]

    if win_amount > 0:
        user["balance"] += win_amount
        user["total_won_coins"] += win_amount
        final_lines.append(f"🎉 <b>Выиграл!</b> Получено <b>{win_amount}</b> монет ({win_desc})")
        ...
        final_lines.append(f"😢 <b>Проиграл!</b> Потеряно 1000 монет.")
...
# --- АДМИН ЗАКРЕП (РЕКЛАМА ЖЕ БИЛДИРҮҮЛӨРДҮ ТОКТООСУЗ ЗАКРЕПТӨӨ) ---
...
async def pin_message_admin(message: types.Message):
    ...
    try:
        ...
        await message.answer("📌 Билдирүү (реклама) ийгиликтүү закрептелди!")
    except Exception as e:
        await message.answer(f"❌ Закрептөө катасы: {e}")
