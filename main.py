...
async def download_and_send_audio(chat_id, query, context, is_auto=False):
    ...
    if not url:
        if not is_auto: 
            await context.bot.send_message(chat_id=chat_id, text="К сожалению, ничего не найдено 😔")
        return
    ...
    try:
        ...
        if not is_auto:
            await context.bot.send_message(chat_id=chat_id, text="Не удалось скачать аудио из-за защиты YouTube. Попробуйте другую песню.")
