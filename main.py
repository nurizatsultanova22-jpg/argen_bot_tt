...
transfer_limits = {}
bonus_limits = {}
give_money_limits = {} # Башкаларга отметка кылып акча берүүнүн лимити (күнүнө 1 жолу)
...
def get_main_reply_keyboard():
    ...
    builder.row(
        ...
        types.KeyboardButton(text="Стати")
    )
    builder.row(
...
async def cmd_start(message: types.Message):
    text = "Привет! Добро пожаловать в игровой бот."
...
async def cmd_profile(message: types.Message):
    ...
    response = (
        ...
        f"💳 Баланс: **{bal}** 🌒\n"
        f"🎮 Сымалган оюндары: {stats['games_played']}\n"
...
async def cmd_stats_top(message: types.Message):
    ...
    # Көп уткан же көп оюн койгондорду сорттоо
...
# БАШКАЛАРГА РЕПЛАЙ ЖЕ ОТМЕТКА МЕНЕН АКЧА БЕРҮҮ (+10000 күнине 1 жолу)
...
async def add_money_by_reply_or_mention(message: types.Message):
    ...
    if target_user_id:
        ...
        # Эгер жөнөкөй колдонуучу болсо күнине 1 жолу гана бере алат
...
async def play_bandit(message: types.Message):
    ...
    if val in [22, 43, 64]:
        win_options = [1000, 3000, 10000, 30000, 70000]
        win_amount = random.choice(win_options)
        
        # Админге дагы баланс кошулуп көбөйө берет (чексиз өсүү)
        if user_id not in user_balances:
            user_balances[user_id] = 5000
...
async def parse_vabank(message: types.Message):
    ...
    amount = bal if user_id != ADMIN_ID else 1000000 # Админ ва-банк койсо
    if target.isdigit():
        color = target
...
async def spin_roulette(message: types.Message):
    ...
    # Видео анимация (Сиз жөнөткөн крипто монеталар төгүлгөн видео file_id же URL аркылуу)
    video_source = "https://www.learningcontainer.com/wp-content/uploads/2020/05/sample-mp4-file.mp4" # Бул жерге өз видеоңуздун шилтемесин же file_id коёсуз
    video_msg = None
    try:
        video_msg = await message.answer_video(video=video_source, caption="🎲 Рулетка крутится...")
    except:
        video_msg = await message.answer("🎲 [Видео рулетки вращается...]")
    ...
    for u_id, bet in active_bets_copy.items():
        ...
        if is_win:
            # Адимндин дагы балансы утканда кошулуп көбөйө берет
