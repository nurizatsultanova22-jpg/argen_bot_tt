...
async def search_music_options(message: types.Message):
    ...
    try:
        def download_file():
            ...
            # SoundCloud үчүн стандарттуу эмес изрөө катасын четтетүү максатында
            # ytsearch ордуна SoundCloud URL аркылуу же авто-сурам колдонобуз
            search_url = f"ytsearch1:audio {query}"  # SoundCloud бөгөт койсо дагы музыканы башка базадан таап берет
    ...
    if not success or not os.path.exists(filename):
        await message.answer("❌ Трек не найден. Попробуйте написать другое название.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        return
