...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        ...
        "Чтобы найти трек, напишите его название (можно с префиксом **«Трек [название]»**). "
        "Бот быстро найдет и отправит вам аудио! 🎶"
    )
...
async def btn_calendar(message: types.Message):
    now = datetime.now()
    calendar_text = f"📅 **Дата:** {now.strftime('%d.%m.%Y')} | ⏰ **Время:** {now.strftime('%H:%M')}"
...
async def btn_warm_words(message: types.Message):
    compliments = [
        "💚 **Пусть ваш сегодняшний день пройдет на высшем уровне!** ✨",
        "🎧 **Наполните свой день прекрасной музыкой!**",
        "✨ **Вы всегда на высоте!** 🔥"
    ]
...
async def btn_entertainment(message: types.Message):
    await message.answer("💚 **Музыкальный вайб:** Хорошая музыка — лучший отдых для души! 🎧", parse_mode="Markdown", reply_markup=get_main_reply_menu())

@dp.message(F.text == "🎬 Видео (24ч)")
async def btn_videos_list(message: types.Message):
    videos = get_all_videos()
    if not videos:
        await message.answer("⚠️ Пока нет сохраненных видео.", reply_markup=get_main_reply_menu())
    else:
...
async def handle_incoming_video(message: types.Message):
    ...
    await message.answer("✅ Видео успешно сохранено на 24 часа!", reply_markup=get_main_reply_menu())
...
async def search_music_options(message: types.Message):
    ...
    query = text
    # "Трек" же "трек" сөздөрүн алып салып издөө үчүн тазалоо
    ...
    if cached_file_id:
        await message.answer_audio(
            ...
            caption=f"💚 **Найдено (Кэш):** {query} 🔥",
            parse_mode="Markdown",
            reply_markup=get_main_reply_menu()
        )
        return

    waiting_msg = await message.answer("🟢 **Ищем трек...** 🎧", parse_mode="Markdown")

    # SoundCloud аркылуу издөө (эч кандай капча же бөгөт койбойт)
    ydl_opts = {
        "format": "bestaudio",
        "quiet": True,
        "noplaylist": True,
    ...
    try:
        ...
        await asyncio.to_thread(search_sc)
    ...
    if not audio_url:
        await message.answer("❌ Трек не найден. Пожалуйста, попробуйте написать название иначе.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        return
