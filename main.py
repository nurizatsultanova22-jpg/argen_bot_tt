...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    welcome_text = (
        "🟢 **Spotify Музыкалык Ботко кош келиңиз!** 🎧\n\n"
        "Ыр издөө үчүн каалаган тректин атын жазыңыз, же **'трек [аты]'** / **'найти [аты]'** форматтарын колдонуңуз. "
        "Бот сизге Spotify стилинде 3 вариантты тез таап берет! 🎶"
    )
...
async def btn_entertainment(message: types.Message):
    await message.answer("💚 **Spotify Музыкалык Вайб:** Жакшы музыка — жан дүйнөнүн эс алуусу! 🎧", parse_mode="Markdown", reply_markup=get_main_reply_menu())
...
# --- SPOTIFY СТИЛИНДЕГИ ИЗДӨӨ ЖАНА ЖҮКТӨӨ ---
@dp.message()
async def search_music_options(message: types.Message):
    ...
    if cached_file_id:
        await message.answer_audio(
            ...
            caption=f"💚 **Spotify (Кэш):** {query} (HQ) 🔥",
            parse_mode="Markdown",
            reply_markup=get_main_reply_menu()
        )
        return

    waiting_msg = await message.answer("🟢 **Spotify: Тректер изделүүдө (3 вариант)...** 🎧", parse_mode="Markdown")
    ...
    try:
        def search_tracks():
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                # Deezer/Open API аркылуу 401 катасыз 3 вариант таап беребиз
                results = ydl.extract_info(f"dzsearch3:{query}", download=False)
                return results.get("entries", [])

        entries = await asyncio.to_thread(search_tracks)

        if not entries:
            await message.answer("❌ Spotify базасынан эч нерсе табылган жок.", parse_mode="Markdown", reply_markup=get_main_reply_menu())
        ...
        for idx, entry in enumerate(entries[:3], 1):
            ...
            artist = entry.get("artist", "Spotify Music")
            full_title = f"{artist} - {title}"
            url = entry.get("url")
            if url:
                builder.row(InlineKeyboardButton(text=f"🟢 Вариант {idx}: {full_title[:35]}...", callback_data=f"dl_{url[:50]}"))

        await bot.delete_message(message.chat.id, waiting_msg.message_id)
        
        if builder.buttons:
            await message.answer(
                f"🟢 **'{query}'** боюнча Spotify варианттары табылды. Тандаңыз:",
                parse_mode="Markdown",
        ...
        try:
            ...
            pass
        await message.answer("⚠️ Ката кетти. Кайра аракет кылыңыз.", parse_mode="Markdown", reply_markup=get_main_reply_menu())

@dp.callback_query(F.data.startswith("dl_"))
async def download_selected_track(callback: types.CallbackQuery):
    track_url = callback.data[3:]
    
    await callback.message.edit_text("⏳ **Spotify треги жогорку сапатта жүктөлүүдө...** 🎧", parse_mode="Markdown")
    ...
    try:
        ...
        if os.path.exists(real_file):
            audio_file = FSInputFile(real_file, filename="spotify_track.mp3")
            sent_msg = await callback.message.answer_audio(
                audio=audio_file,
                caption="💚 **Spotify: Ийгиликтүү жүктөлдү! (320kbps)** 🔥\n🎧 Угуп рахат алыңыз!",
                parse_mode="Markdown",
                reply_markup=get_main_reply_menu()
            )
            if sent_msg.audio:
