...
# АНТИМАТ ФИЛЬТР (Орусча сөгүнгөндө эскертүү берүү)
@dp.message(F.text)
...
async def cmd_balance_simple(message: types.Message):
    ...
    await message.answer(f"KNK-{user_name}\nмонеты: {bal} 🌒")
...
async def play_bandit(message: types.Message):
    ...
    if val in [22, 43, 64]:
        win_options = [1000, 3000, 10000, 30000]
...
# СТАТИСТИКА (стати / статика / топ) — эң көп монетасы барлар
...
# ОТМЕТКА МЕНЕН 10000 БЕРҮҮ (Суткасына 1 гана жолу)
...
async def add_ten_thousand(message: types.Message):
    ...
    if sender_id in plus_limits:
        ...
        if current_time - last_time < 86400:
            ...
            await message.reply(f"❌ Вы можете давать +10000 монеталар суткасына 1 гана жолу! Подождите еще {hours_left} ч.")
            return
...
# СТАВКАЛАРДЫ КАТТОО (Бир адам бир нече ставка кошо алат, өчүрүлбөйт)
