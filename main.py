...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "🔍 Просто отправь название песни, и я её найду!\n"
...
async def download_and_send_audio(chat_id, query, context):
    ...
    # Текстсиз, жалаң эмодзи менен издөө процессин көрсөтүү
    status_msg = await context.bot.send_message(chat_id=chat_id, text="🔍 🎧 ⏳...")
...
async def start_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🎲 1-ден 10го чейин сан ойлодум. Таап көр, цифра жаз! 🤔")
    ...
    if chat_id in user_games:
        try:
            ...
            if guess == user_games[chat_id]:
                await update.message.reply_text("🎉 🏆 Туура таптың!")
                del user_games[chat_id]
            else:
                await update.message.reply_text("❌ 🙈 Кайрадан байкап көр!")
        except: pass
