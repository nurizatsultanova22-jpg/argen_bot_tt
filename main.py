...
# ОТМЕТКА / РЕПЛАЙ КОДУ БАЛАНС КОШУУ (@user + 10000 же реплай кылып +10000)
@dp.message(F.text.regexp(r"^[\+\+\s]*(\d+)$"))
async def add_money_by_reply_or_mention(message: types.Message):
    # Текшерүү: Админ гана же каалаган адам кошо алабы (админ же башкалар)
    ...
    # Эгер башка адамдын билдирүүсүнө реплай (жооп) кылса
    if message.reply_to_message and message.reply_to_message.from_user:
        ...
        for entity in message.entities:
            if entity.type == "text_mention":
                ...
                # сөз менен @username болсо (базада болбосо табуу кыйын болгондуктан реплай ыңгайлуу)
                pass
...
async def parse_vabank(message: types.Message):
    ...
    # БАРДЫК КОЛДОНУУЧУЛАРДЫКИН КАТТОО (Каалаган адам ставка коё алат)
...
async def parse_bet(message: types.Message):
    ...
    # БАРДЫК КОЛДОНУУЧУЛАРДЫКИН КАМТАС ОПЕРАЦИЯСЫ
    current_bets[user_id] = {"amount": amount, "color": color, "name": user_name}
    stats = get_user_stats(user_id)
    if amount > stats['max_bet']:
        stats['max_bet'] = amount
...
# КИМ ДИР БИРӨӨ «ГО» ДЕСЕ БАРДЫК КОЛДОНУУЧУЛАРДЫН СТАВКАЛАРЫ ОЙНОЛОТ
...
async def spin_roulette(message: types.Message):
    ...
    # Сүрөттөгүдөй GIF анимациясы (3 секунд айланат)
    ...
    # Бардык ставка койгондорду бирдей текшерип чыгуу
