...
def search_tracks(query):
    ...
    # 2. Эгер аз чыкса же чыкпаса, yt_dlp аркылуу SoundCloud/Web аркылуу толук шилтемелерди алабыз
    if len(results) < 5:
        try:
            ydl_opts = {
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                for entry in res.get('entries', []):
                    ...
                    if webpage_url and not any(r['title'].lower() == title.lower() for r in results):
                        results.append({
                            'title': title,
                            'url': webpage_url,
                            'thumbnail': thumb
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("select_"):
        ...
        status_msg = await query.message.reply_text(f"⚡ Скачиваю и отправляю: *{title}* ⏳", parse_mode="Markdown")

        try:
            audio_url = page_url
            # Эгер шилтеме толук веб-страница болсо, yt_dlp аркылуу түздөн-түз mp3 шилтемесин чыгарып алабыз
            if "http" in page_url and not page_url.endswith((".mp3", ".m4a")):
                ydl_opts = {'format': 'bestaudio/best', 'quiet': True, 'noplaylist': True}
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                    info = ydl.extract_info(page_url, download=False)
                    audio_url = info.get('url')

            song_database[title.lower()] = {'title': title, 'url': audio_url, 'thumbnail': thumbnail}
            ...
            await status_msg.edit_text("❌ Не удалось отправить аудиофайл. Попробуйте другой трек.")
