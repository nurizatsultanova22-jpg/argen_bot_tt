...
async def cmd_start(message: types.Message):
    await message.answer("👋 Здравствуйте! Бот работает. Напишите «Профиль», «Бандит» или «Рулетка».")
...
async def slot_bandit_game(message: types.Message):
    ...
    # Логгообмен для бандита
    user["roulette_history"].insert(0, f"Бандит: {final_display} | {result_text}")
...
async def start_game_handler(message: types.Message):
    ...
    result_lines = [
        f"🎯 Рулетка: выпало {rolled_num} {rolled_emoji}",
        f"👤 <b>{username}</b> ставил {bet_amt} {bet_name}"
    ]
    ...
    if is_win:
        ...
        result_lines.append(f"✅ <b>Выигрыш:</b> +{winnings} 🪙")
        ...
        result_lines.append(f"❌ <b>Проигрыш:</b> -{bet_amt} 🪙")

        if bet_amt > global_records["max_loss"]["amount"]:
            global_records["max_loss"] = {"name": username, "amount": bet_amt}

    # Логдор үчүн сактоо
    history_text = f"Рулетка: {rolled_num} {rolled_emoji} | Ставка: {bet_amt} {'Победа' if is_win else 'Проигрыш'}"
    user_data_dict["roulette_history"].insert(0, history_text)
...
async def show_game_logs(message: types.Message):
    ...
    if user_id not in users_data or not users_data[user_id]["roulette_history"]:
        await message.answer("📜 История игр пуста. Сыграйте в Рулетку или Бандит!")
        return

    history = users_data[user_id]["roulette_history"]
    history_display = "📜 <b>Ваша история игр:</b>\n\n" + "\n".join([f"• {item}" for item in history])
